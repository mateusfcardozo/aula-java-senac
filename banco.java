/*
A função "banco(linha 20 a 160)" possui um método main, que inicia a execução do programa, e outras três funções: cadastrarCliente, realizarLogin e atualizarCliente.

A função "cadastrarCliente(linha 25 a 46)" permite cadastrar um novo cliente no sistema bancário. Ele solicita ao usuário que informe o nome, CPF, senha e celular do cliente. Se o número de clientes cadastrados ultrapassar o limite de 10,
...
A função "realizarLogin(linha 48 a 65)" permite que um cliente faça login no sistema. Ele solicita que o usuário informe seu CPF e senha, e verifica se há um cliente com esses dados cadastrado no sistema. Se houver, a função exibe o nome,
...
A função "atualizarCliente(linha 67 a 130" permite que um cliente atualize seus dados cadastrados no sistema. Ele exibe um menu com as opções disponíveis: atualizar nome, atualizar senha, atualizar celular, adicionar saldo e voltar.
...
A função "main(linha 132 a 159)" serve para o usuario escolher umas das opções e ira mandar para uma das funções.
   ...
   public static void atualizarCliente(int auxiliarCliente){
        ...
        switch(opcao){
            ...
            case 5:
                ...
                String valorRetirar = scanner.nextLine();
                double saldoRetirar = Double.parseDouble(clientes[auxiliarCliente][4]);
                double novoSaldoRetirada = saldoRetirar - Double.parseDouble(valorRetirar);
                clientes[auxiliarCliente][4] = String.format("%.2f", novoSaldoRetirada);
                if(novoSaldoRetirada < 0){
